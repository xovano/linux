				     ...
				     const char *buf, size_t len)
{
	...
	case CDDC_NCO_FREQ:
		...
		if (chan->output) {
			...
			ret = adi_ad9081_adc_ddc_coarse_nco_set(&phy->ad9081,
								cddc_mask, readin);
			if (!ret)
				phy->rx_cddc_shift[cddc_num] = readin;
			else
				ret = -EFAULT;
		}
		break;
	...
	case CDDC_NCO_PHASE:
		...
		if (chan->output) {
			...
			if (!ret)
				phy->rx_cddc_phase[cddc_num] = readin;
			else
				ret = -EFAULT;
		}
		break;
...
static void ad9081_work_func(struct work_struct *work)
{
	...
	if (!(status & BIT(6))) {
		...
		phy->jesd_tx_link.lane_cal_rate_kbps = 0;
		jesd204_fsm_stop(phy->jdev, JESD204_LINKS_ALL);
		jesd204_fsm_clear_errors(phy->jdev, JESD204_LINKS_ALL);
	...
	size_t count, loff_t *ppos)
{
	...
	if (entry->out_value) {
		...
		case DBGFS_BIST_PRBS_JRX_ERR:
			mutex_lock(&indio_dev->mlock);
			for (i = 0; i < phy->jesd_tx_link.jesd_param.jesd_l; i++) {
				adi_ad9081_prbs_test_t prbs_rx_result;

				for (j = 0; j < 8; j++)
					if (phy->jesd_tx_link.logiclane_mapping[j] == i) {
	...
	const char __user *userbuf, size_t count, loff_t *ppos)
{
	...
	case DBGFS_BIST_PRBS_JTX:
		...
		if (val == 0)
			...
			adi_ad9081_jesd_tx_phy_prbs_test(&phy->ad9081,
				AD9081_LINK_0, ad9081_val_to_prbs(val));
		entry->val = val;
		mutex_unlock(&indio_dev->mlock);
	...
	case DBGFS_BIST_JRX_SPO_SWEEP:
		...
		ret = adi_ad9081_jesd_rx_spo_sweep(&phy->ad9081, val & 0x7,
				...
				ad9081_deserializer_mode_get(&phy->jesd_tx_link),
				val3, &lv, &rv);
		if (ret) {
			mutex_unlock(&indio_dev->mlock);
			return ret;
				      ...
				      bool complex, bool buffer_capable)
{
	...
	num_labels = (m * phy->multidevice_instance_count);
	num_labels += phy->jesd_tx_link.jesd_param.jesd_m *
		      (phy->jesd_tx_link.jesd_param.jesd_duallink > 0 ? 2 : 1);
	phy->labels = devm_kcalloc(&phy->spi->dev, num_labels, sizeof(char*), GFP_KERNEL);
	if (!phy->labels)
		return -ENOMEM;
	...
	for (c = 0, i = 0; i < (m * phy->multidevice_instance_count);
		...
		if (i < m) {
			...
			phy->labels[c] = ad9081_lable_writer(phy, &phy->chip_info.channel[c]);
		} else {
			phy->labels[c] = "buffer_only";
		}
	}
